 ...
 * Jobs will be queued to handle each enemy's pathfinding once the enemy is within a specified range.
*/
...
public class AIManager : MonoBehaviour
{
    ...
    [SerializeField] private Tilemap unWalkableMap;
    ...
    private void GenerateNodes()
    {
        ...
        // Goes through and removes all nodes that aren't walkable
        for (int x = 0; x < mapBounds.size.x; x++)
        {
            for (int y = 0; y < mapBounds.size.y; y++)
            {
                if (unWalkableMap.GetTile(new Vector3Int(x - Mathf.Abs(mapBounds.position.x), y - Mathf.Abs(mapBounds.position.y), 0)) != null)
                {
                    nodes.Remove(WorldPosToGrid(x - Mathf.Abs(mapBounds.position.x) + offset, y - Mathf.Abs(mapBounds.position.y) + offset));
                }
            }
        }
